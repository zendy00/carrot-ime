...
- **유휴 임계 조정(최종 약 20초), 포커스 즉시표시 제거:** 포커스만으로는 뜨지 않고(포커스 시점부터 유휴 카운트 시작), 약 20초 이상 키 입력이 없어야 표시. 입력 시작 시 숨김. `IdleReappearMs=20_000`, `TrackActivity` 포커스 리셋을 유휴가 아니라 활동시작으로 변경. (5초→1분→20초로 사용자 피드백 따라 조정)
